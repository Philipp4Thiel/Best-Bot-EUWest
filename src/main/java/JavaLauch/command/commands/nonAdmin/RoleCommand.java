...
public class RoleCommand implements IPublicCommand {
    ...
    public void handlePublic(CommandContext ctx) {
        ...
        if (subCommand.startsWith("<@!") && subCommand.endsWith(">")) {
            ...
            if (!member.getId().equals(id)) {
                ctx.getChannel().sendMessage("something weird happened");
                return;
            }
            if (member == null) {
    ...
    void addRole(CommandContext ctx, String role) throws FileNotFoundException {
        ...
        ctx.getChannel().sendMessage(EmbedUtils.getDefaultEmbed()
                .setTitle("Success")
                .setDescription("Successfully added `" + role + "` to the available roles.")
                .build()).queue();
    }
    ...
    void removeRole(CommandContext ctx, String role) throws FileNotFoundException {
        if (roles.containsKey(role)) {
            ...
            ctx.getChannel().sendMessage(EmbedUtils.getDefaultEmbed()
                    .setTitle("Success")
                    .setDescription("Successfully removed `" + role + "` from the available roles.")
                    .build()).queue();
            return;
        }
    ...
    void signUp(CommandContext ctx, String role) throws FileNotFoundException {
        ...
        ctx.getChannel().sendMessage(EmbedUtils.getDefaultEmbed()
                .setTitle("Success")
                .setDescription("Successfully signed up for `" + role + "`.")
                .build()).queue();
    }
    ...
    void quit(CommandContext ctx, String role) throws FileNotFoundException {
        ...
        ctx.getChannel().sendMessage(EmbedUtils.getDefaultEmbed()
                .setTitle("Success")
                .setDescription("Successfully quited  `" + role + "`. :sadge:")
                .build()).queue();
    }
    ...
    void ping(CommandContext ctx, String role) {
        ...
        memberIDs.stream().forEach(
                (it) -> allMentions.append("<@").append(it).append(">")
        );
        ...
        ctx.getChannel().sendMessage(EmbedUtils.getDefaultEmbed()
                ...
                .addField("", "If you got wrongfully pinged submit a bug with the bug report feature.", false)
                .build()).queue();
    }
    ...
    EmbedBuilder publicHelpRaw(String prefix) {
        ...
        embed.addField("", new ColoredStringAsciiDoc()
                ...
                .addOrange(prefix + "role")
                .build(), false);

        // all
        embed.addField("", new ColoredStringAsciiDoc()
                .addBlueAboveEq("all roles:")
                .addNormal("With this you get a list of all roles available.")
                .addOrange(prefix + "role all")
                .build(), false);

        // signup
        embed.addField("", new ColoredStringAsciiDoc()
                .addBlueAboveEq("sign up for a role:")
                .addNormal("With this you can sign up for a role.")
                .addOrange(prefix + "role signup <role>")
                .build(), false);

        // quit
        embed.addField("", new ColoredStringAsciiDoc()
                .addBlueAboveEq("quit a role:")
                .addNormal("With this you can get rid of a role.")
                .addOrange(prefix + "role quit <role>")
                .build(), false);

        // ping
        embed.addField("", new ColoredStringAsciiDoc()
                .addBlueAboveEq("ping a role:")
                .addNormal("With this you can ping all members of a role.")
                .addOrange(prefix + "role ping <role>")
                .build(), false);

        // @member
        embed.addField("", new ColoredStringAsciiDoc()
                .addBlueAboveEq("Inspect:")
                .addNormal("With this you can see what roles somebody has.")
                .addOrange(prefix + "role <@member>")
                .build(), false);
    ...
    void roleAlreadyExistsError(CommandContext ctx, String role) {
        ctx.getChannel().sendMessage(EmbedUtils.getDefaultEmbed()
                .setTitle("Error")
                .setDescription("The role `" + role + "` does already exits.")
                .build()
        ).queue();
    }
    ...
    void roleIsEmptyError(CommandContext ctx, String role) {
        ctx.getChannel().sendMessage(EmbedUtils.getDefaultEmbed()
                .setTitle("Error")
                .setDescription("The role `" + role + "` has no members.")
                .build()
        ).queue();
    }
    ...
    void thisGuyDoesntHaveARole(CommandContext ctx) {
        ctx.getChannel().sendMessage(EmbedUtils.getDefaultEmbed()
                .setTitle("Error")
                .setDescription("This user has no roles.")
                .build()
        ).queue();
    }
    ...
    void thereAreNoRoles(CommandContext ctx) {
        ctx.getChannel().sendMessage(EmbedUtils.getDefaultEmbed()
                .setTitle("Error")
                .setDescription("There are no roles on this server.\nAsk me or admins to add roles you'd like to have.")
                .build()
        ).queue();
    }
    ...
    void notEnoughArgs(CommandContext ctx, String missingArg) {
        ctx.getChannel().sendMessage(EmbedUtils.getDefaultEmbed()
                .setTitle("Error")
                .setDescription("There seems to be a missing argument.\nMake sure your command has a valid: `" + missingArg + "`")
                .build()
        ).queue();
    }
    ...
    void errorWithJson(CommandContext ctx) {
        ctx.getChannel().sendMessage(EmbedUtils.getDefaultEmbed()
                .setTitle("Error")
                .setDescription("something went wrong while writing into the jsonfile")
                .build()
        ).queue();
    }
}
